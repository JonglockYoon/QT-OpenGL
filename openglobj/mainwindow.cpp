    ...
    : QMainWindow(parent), ui(new Ui::MainWindow)
{
    ...
    readFileAct = new QAction(QIcon(), tr("read&File"), this);
    connect(readFileAct, SIGNAL(triggered()), this, SLOT(setReadFile()));
    exitAct = new QAction(QIcon(), tr("E&xit"), this);
...
void MainWindow::setReadFile()
{
    aiString filter_temp;
    QString filename, filter;

    mGLView->bFileOpening = true;
    ...
    filename = QFileDialog::getOpenFileName(this,
            "Choose the file", QDir::currentPath(),
            filter);
    if(filename.isEmpty()) {
        mGLView->bFileOpening = false;
        return;
    }
...
//    QTime time_begin = QTime::currentTime();
    ...
    mScene = mImporter.ReadFile(filename.toStdString(), aiProcess_Triangulate | aiProcess_GenNormals | aiProcess_ValidateDataStructure | \
    ...
    if(mScene != nullptr)
    {
        ...
        //
        mGLView->SetScene(mScene, filename);
        // Select first camera
